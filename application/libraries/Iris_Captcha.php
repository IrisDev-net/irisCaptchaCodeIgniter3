...
class Iris_Captcha
{
    ...
    Public function __construct() {
		...
		$this->ci->load->config('iriscaptcha', TRUE);
		
		
        $this->Secret = $this->ci->config->item('IrisCaptcha_Secret_Key', 'iriscaptcha');
        $ss = explode("0x",$this->ci->config->item('IrisCaptcha_Secret_Key', 'iriscaptcha'));
        $this->UniqId = "0x".end($ss);
        $this->PublicKey = $this->ci->config->item('IrisCaptcha_Publick_Key', 'iriscaptcha');
    ...
    Private function Check_Answer_Remote ( $response, $remoteip, $extra_params = array()){
            ...
            if ($this->ci->config->item('IrisCaptcha_Developing_Mode', 'iriscaptcha')==TRUE)
            {
                $remoteip=''; 
            }
